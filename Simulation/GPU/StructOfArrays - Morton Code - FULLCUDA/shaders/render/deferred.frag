...
layout (location = 0) out vec4 front_depth;
layout (location = 1) out vec4 back_depth;
//out vec4 outColor;
...
void main()
{
    if (distance(iuv,vec2(0.5,0.5))<=0.5){
        ...
        front_depth   = vec4(1-ndc_depth);
        back_depth    = vec4(far_ndc_depth);
        //outColor =      vec4(linearize_depth(ndc_depth,0.02,10));
    }
    else
        discard;
}
...
float getheight(vec2 pos,vec2 rep){
    float heightmult = 0.1;
    //float y = (pnoise(pos,rep));
    //y =  y* smoothstep(2.1,1.6,abs(pos.x))*smoothstep(2.9,2.0,abs(pos.y));
    return heightmult* clamp(
        texture(height,clamp(((pos.xy-voxmin.xz)/(voxmax.xz-voxmin.xz)).yx,vec2(0.0),vec2(1.0))).x
        ,0.0,0.35);//*smoothstep(0,1 ,abs(y) );
}
...
bool colisionCheck( vec3 current,vec3 rayDir,inout vec3 color)
{
    ...
    if ( ((current + 2*rayDir).y < -1.99+y )){
        ...
        color = intensity*texture(ground,(clamp(((pos.xy-voxmin.xz)/(voxmax.xz-voxmin.xz)).yx,vec2(0.0),vec2(1.0)))).xyz;//vec3(0.6);
